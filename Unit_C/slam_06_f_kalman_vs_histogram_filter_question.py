...
def kalman_filter_step(belief, control, measurement):
    ...
    # Prediction.
    new_mu = belief.mu + control.mu
    new_sigma2 = belief.sigma2 + control.sigma2

    prediction = Density(new_mu, new_sigma2)  # Replace

    # Correction.
    kalman_gain = prediction.sigma2/(prediction.sigma2 + measurement.sigma2)
    corrected_mu = prediction.mu + kalman_gain * \
        (measurement.mu - prediction.mu)
    corrected_sigma2 = prediction.sigma2 * (1 - kalman_gain)
...
if __name__ == '__main__':
    arena = (0, 200)
    # Distribution.triangle or Distribution.gaussian.
    ...
    for i in range(len(controls)):
        # Histogram
        (prediction, position) = histogram_filter_step(
            position, controls[i], measurements[i])
        histogram_plot(prediction, measurements[i], position)
        # Kalman
        (prediction_, position_) = kalman_filter_step(
            position_, controls_[i], measurements_[i])
